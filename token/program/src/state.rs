...
#[repr(C)]
#[derive(Clone, Copy, Debug, Default, PartialEq)]
pub struct MintExtra {
    /// total supply of tokens on l1 (should always be more than supply)
    pub supply_on_l1: COption<u64>,
}
impl Sealed for MintExtra {}
impl IsInitialized for MintExtra {
    fn is_initialized(&self) -> bool {
        self.supply_on_l1.is_some()
    }
}
impl Pack for MintExtra {
    const LEN: usize = 8 + 4;

    fn unpack_from_slice(src: &[u8]) -> Result<Self, ProgramError> {
        let src = array_ref![src, 0, MintExtra::LEN];
        let supply_on_l1 = src;
